...
* Automated the data collection process into CSV format and generated **Rate-Distortion (R-D) curves** to visualize the trade-off between compression rate and reconstructed image quality.
 ...
 ### 🔹 LSTM_COMPARE_METRICS
* Multi-Model Air Pollution Forecasting: Developed and compared five deep learning architectures (LSTM, BiLSTM, GRU, ConvLSTM, and Attention-LSTM) to predict hourly $PM2.5$ air pollution levels.
...
### 🔹Semi-Supervised Anomaly Detection: The model is trained exclusively on normal transactions to learn the "essence" of legitimacy, allowing it to identify fraud as any data that fails to be accurately reconstructed (High Reconstruction Error).
